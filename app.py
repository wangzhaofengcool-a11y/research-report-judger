...
if not has_keys:
    ...
    mode=st.radio("处理模式",["逐一分析","合并分析"],horizontal=True,key="mode",label_visibility="collapsed")
    ci=st.text_area("自定义要求（可选）",placeholder="例如: 关注第三章方法论 · 对比2024年后数据",height=80,key="ci",label_visibility="collapsed")

    c1,c2=st.columns([3,1])
    with c1:go=st.button("开始审校",type="primary",disabled=not up or st.session_state.running,use_container_width=True)
    with c2:
...
if st.session_state.history and not st.session_state.running:
    with st.expander(f"历史 ({len(st.session_state.history)})"):
        for i,h in enumerate(reversed(st.session_state.history)):
            if st.button(f"{h['time']} {h['title']}",key=f"hist_{i}",use_container_width=True):
                st.session_state.reports=[{"doc":h['title'],"report":h['report'],"results":h.get('results',[]),"search":h.get('search',{}),"time":h['time']}]
                st.session_state.all_results=[];st.session_state.done=True;st.rerun()

# ── Pipeline ──
if st.session_state.running and not st.session_state.done and has_keys:
    st.divider();st.subheader("分析进度")
    pbar=st.progress(0);stat=st.empty();det=st.expander("详情",expanded=True)
    try:
        ...
        for gidx,(doc_info,doc_text) in enumerate(doc_groups):
            st.caption(f"{gidx+1}/{total_groups} {doc_info['fn']} ({len(doc_text):,}字符)")
            bp=12+int(83*(gidx/total_groups));gp=83//total_groups

            stat.info(f"提取结论{gidx+1}/{total_groups}");pbar.progress(bp+int(gp*.2))
            ...
            for pid,pk,pm in models:
                try:content=call_llm(pid,pk,pm,S2,U2);gr.append({"p":pid,"m":pm,"c":content,"e":None})
                except Exception as e:gr.append({"p":pid,"m":pm,"c":"","e":str(e)})
            all_results.append({"doc":doc_info['fn'],"results":gr,"search":sr})
            pbar.progress(bp+int(gp*.8))

            mt="\n".join(f"##{r['p'].upper()}\n{r['c'][:4000]}"for r in gr)
...
if st.session_state.done and st.session_state.reports:
    st.divider();st.subheader("审校结果")
    reports=st.session_state.reports
    if len(reports)>1:
        tabs=st.tabs([r['doc'][:25] for r in reports])
        for i,(tab,rpt) in enumerate(zip(tabs,reports)):
            with tab:
                st.caption(f"{sum(len(v)for v in rpt['search'].values())}条检索")
        ...
        c1,c2=st.columns(2)
        c1.metric("模型",len(rpt['results']));c2.metric("检索",f"{tsc}条")
        t1,t2=st.tabs(["报告","详情"])
        with t1:
            st.markdown('<div class="report-box">',unsafe_allow_html=True)
            st.markdown(rpt['report']);st.markdown('</div>',unsafe_allow_html=True)
            ts=datetime.now().strftime("%Y%m%d_%H%M%S")
            st.download_button("下载报告",data=rpt['report'].encode(),file_name=f"审校报告_{ts}.md",mime="text/markdown",use_container_width=True)
        with t2:
            for rr in rpt['results']:
