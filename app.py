...
if not has_keys:
    st.warning("请先在 Settings > Secrets 中配置 API Key")
    ...
    with st.sidebar:
        ...
        models=[]
        if ds_key:
            st.checkbox("DeepSeek V4 Pro",value=True,disabled=True,key="mds")
            models.append(("deepseek",ds_key,ds_model))
        if gk:
            if st.checkbox("Gemini 3.5 Flash",value=False,key="mgg"):
                models.append(("google",gk,sk("GOOGLE_MODEL","gemini-2.5-flash")))
        if glk:
            if st.checkbox("GLM 5.1 Pro",value=False,key="mglm"):
                models.append(("glm",glk,sk("GLM_MODEL","glm-4.5")))
        st.caption(f"已选 {len(models)} 个")
        st.markdown("---")
        st.info("联网检索: DuckDuckGo")
        st.caption("v3.2")
...
def parse_pdf(fb,fn):
    ...
    try:
        ...
        if doc.is_encrypted:
            ...
            except:return{"fn":fn,"txt":"","chars":0,"err":"PDF已加密"}
        ...
        if len(total)<100 and empty>doc.page_count*0.5:
            return{"fn":fn,"txt":"","chars":0,"err":f"文档含{imgs}张图片，文本不足，请用文字版PDF"}
        return{"fn":fn,"txt":total,"chars":len(total),"pages":doc.page_count}
    except Exception as e:
        try:doc.close()
        except:pass
        return{"fn":fn,"txt":"","chars":0,"err":f"解析失败:{str(e)[:80]}"}
...
def parse_docx(fb,fn):
    ...
    except Exception as e:return{"fn":fn,"txt":"","chars":0,"err":f"解析失败:{str(e)[:80]}"}
...
def parse_text(fb,fn):
    ...
    return{"fn":fn,"txt":"","chars":0,"err":"无法识别编码"}
